...
let chosen = color;
if (color === 'auto') {
chosen = room.players.white ? (room.players.black ? null : 'black') :
'white';
}
...
// Put socket in room
socket.join(roomId);
// Assign color if provided and available
...
winner: chess.isCheckmate() ? (chess.turn() === 'w' ? 'black' :
'white') : null
});
}
}
...
io.to(roomId).emit('players', { white: !!room.players.white, black: !!
room.players.black });
}
socket.leave(roomId);
});
...
server.listen(PORT, () => console.log(`Chess server running on http://localhost:
${PORT}`));
